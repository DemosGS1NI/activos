<script>
  ...
  async function loadDocumentTypes() {
    ...
    try {
      ...
      documentTypes = Array.isArray(data) ? data : [];
    } catch (err) {
      showMessage(
        err.message || "No fue posible cargar los tipos de documento",
        "error"
      );
    } finally {
      loading = false;
  ...
  async function saveDocumentType() {
    ...
    try {
      ...
      if (creating) {
        ...
        showMessage("Tipo de documento creado", "success");
      } else if (editingId) {
        await updateDocumentType(editingId, payload);
        showMessage("Tipo de documento actualizado", "success");
      }

      await loadDocumentTypes();
      cancelEdit();
      dispatch("change", { entity: "document-type" });
    } catch (err) {
      showMessage(
        err.message || "No fue posible guardar el tipo de documento",
        "error"
      );
    } finally {
      submitting = false;
  ...
  async function removeDocumentType(item) {
    const confirmed = confirm(
      `¿Eliminar el tipo de documento "${item.name || item.code}"?`
    );
    if (!confirmed) return;
    submitting = true;
    resetMessage();
    try {
      ...
      showMessage("Tipo de documento eliminado", "success");
      await loadDocumentTypes();
      dispatch("change", { entity: "document-type" });
    } catch (err) {
      showMessage(
        err.message || "No fue posible eliminar el tipo de documento",
        "error"
      );
    } finally {
      submitting = false;
...
<div class="space-y-6">
  <div class="flex items-center justify-between gap-4">
    <div>
      <h1 class="text-xl font-semibold text-sky-900">Tipos de documento</h1>
    ...
    <div class="overflow-x-auto">
      <table class="min-w-full border-collapse">
        ...
        <tbody class="text-sm text-sky-900">
          ...
          {#each sortedDocumentTypes() as item (item.id)}
            <tr
                ...
                <td class={`${DATA_CELL_CLASS} text-right`}>
                  <div class="flex justify-end gap-2">
                    ...
                    <button
                      ...
                      on:click={() => removeDocumentType(item)}
                      type="button"
                      disabled={submitting}
          ...
          {#if !loading && !documentTypes.length && !creating}
            <tr>
              <td
                colspan="4"
                class="px-3 py-6 text-center text-sm text-sky-900"

<script>
  ...
  onMount(async () => {
    await Promise.all([loadDepartments(), loadCostCenters()]);
  });
  ...
  async function loadCostCenters() {
    ...
    try {
      ...
      costCenters = Array.isArray(data) ? data : [];
    } catch (err) {
      showMessage(
        err.message || "No fue posible cargar los centros de costo",
        "error"
      );
    } finally {
      loading = false;
  ...
  function buildPayload() {
    return {
      ...
      department_id: normalizedDepartment(draft.department_id),
    };
  }
  ...
  async function saveCostCenter() {
    ...
    try {
      ...
      if (creating) {
        ...
        showMessage("Centro de costo creado", "success");
      } else if (editingId) {
        await updateCostCenter(editingId, payload);
        showMessage("Centro de costo actualizado", "success");
      }

      await loadCostCenters();
      cancelEdit();
      dispatch("change", { entity: "cost-center" });
    } catch (err) {
      showMessage(
        err.message || "No fue posible guardar el centro de costo",
        "error"
      );
    } finally {
      submitting = false;
  ...
  async function removeCostCenter(item) {
    const confirmed = confirm(
      `¿Eliminar el centro de costo "${item.name || item.code}"?`
    );
    if (!confirmed) return;
    submitting = true;
    resetMessage();
    try {
      ...
      showMessage("Centro de costo eliminado", "success");
      await loadCostCenters();
      dispatch("change", { entity: "cost-center" });
    } catch (err) {
      showMessage(
        err.message || "No fue posible eliminar el centro de costo",
        "error"
      );
    } finally {
      submitting = false;
...
<div class="space-y-6">
  <div class="flex items-center justify-between gap-4">
    <div>
      <h1 class="text-xl font-semibold text-sky-900">Centros de costo</h1>
      <p class="text-sm text-sky-700">
        Administra los centros de costo asociados a los departamentos.
      </p>
    </div>
    <button
    ...
    <div class="overflow-x-auto">
      <table class="min-w-full border-collapse">
        ...
        <tbody class="text-sm text-sky-900">
              ...
              <td class={`${DATA_CELL_CLASS} text-right`}>
                <div class="flex justify-end gap-2">
                  <button
                    ...
                    on:click={saveCostCenter}
                    disabled={submitting}
                    type="button"
                    aria-label="Guardar centro de costo"
                  >
                    {@html icons.check}
                  </button>
                  <button
          ...
          {#each sortedCostCenters() as item (item.id)}
            <tr
                ...
                <td class={`${DATA_CELL_CLASS} text-right`}>
                  <div class="flex justify-end gap-2">
                    ...
                    <button
                      ...
                      on:click={() => removeCostCenter(item)}
                      type="button"
                      disabled={submitting}
          ...
          {#if !loading && !costCenters.length && !creating}
            <tr>
              <td
                colspan="4"
                class="px-3 py-6 text-center text-sm text-sky-900"
